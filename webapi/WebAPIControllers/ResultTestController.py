...
class ResultTestController(AbstractController):
    ...
    @put("/result_test")
    async def update_result_test(self, aId: uuid.UUID, result_test: ResultTestUpdateSchema, user=Depends(get_user)) -> ResultTestViewModel:
        return await self._logic.Update(user, ResultTestViewModel.Update(aId, result_test.completed_date, result_test.note))

    @put("/add_answer")
    async def add_answer_in_result_test(self, result_test_id: uuid.UUID, answer: AnswerSchema, user=Depends(get_user)):
        return await self._logic.Update(user, ResultTestViewModel.GetById(result_test_id)
    ...
    @get("/result_test")
    async def get_result_by_id(self, aId: uuid.UUID, user=Depends(get_user)) -> ResultTestViewModel:
        return await self._logic.Get(user, ResultTestViewModel.GetById(aId))
    ...
    async def get_results_test_by_created_tests(self, user=Depends(get_user)) -> list[ResultTestViewModel]:
        return await self._logic.GetFromCreatedUser(user)
    ...
    async def change_note_in_result_test(self, result_test_id: uuid.UUID, note: str, user=Depends(get_user)):
        return await self._logic.Update(user, ResultTestViewModel.Update(result_test_id, None, note))
    ...
    async def add_marks_for_all_answers(self, answers: list[AnswerWithMark], user=Depends(get_user)):
        return await self._logic.Update(user, ResultTestViewModel.GetById(None)
                                        .AddAnswers([AnswerViewModel.Update(i.answer_id, i.mark) for i in answers]))

    @get("/results_test_by_created_tests_easy")
    async def get_results_test_by_created_tests_in_easy_format(self, user=Depends(get_user)) -> list[ResultTestEasyViewModel]:
        return await self._logic.GetFromCreatedUserInEasyFormat(user)

    @get("/results_test_by_user_easy")
    async def get_results_test_by_user_in_easy_format(self, user=Depends(get_user)) -> list[ResultTestEasyViewModel]:
        return await self._logic.GetFromUserInEasyFormat(user)
